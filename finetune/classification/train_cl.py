...
def main():
    ...
    # pretrain_tag = None
    # number_tag = 0
    # pretrain_tag = "halfyear_5000_1_epochs"
    # # pretrain_tag = "halfyear_5000_2_epochs"
    # number_tag = 1
    # pretrain_tag = "halfyear_10000_1_epochs"
    # # pretrain_tag = "halfyear_10000_2_epochs"
    # number_tag = 2
    pretrain_tag = "halfyear_20000_1_epochs"
    # pretrain_tag = "halfyear_20000_2_epochs"
    number_tag = 3
    ...
    if model_tag == "LSTM":
        ...
        print("Entered wrong model_type")
        return 0
    ...
    if pretrain_tag is not None:
        pre_trained_model = torch.load(f"pretrainmodels/{model_type}_model_ViT_Siamese_with_status_{pretrain_tag}.pt")
    ...
    history = model_fit(
        ...
        save_path=f"new_models/{model_type}_model_{model_tag}_5_class_{pretrain_tag}_{number_tag}.pt" if pretrain_tag is not None else f"new_models/{model_type}_model_{model_tag}_5_class_{number_tag}.pt",
        # save_path="models/{model_tag}_model_5_class.pt",
        device=device,
    )
    model = torch.load(f"new_models/{model_type}_model_{model_tag}_5_class_{pretrain_tag}_{number_tag}.pt" if pretrain_tag is not None else f"new_models/{model_type}_model_{model_tag}_5_class_{number_tag}.pt")
    ...
    print(model_type, model_tag, pretrain_tag, number_tag)
    ...
    if pretrain_tag is not None:
        loss_path = f"new_loss/{model_type}_model_{model_tag}_5_class_{pretrain_tag}_{number_tag}.pkl"
    else:
        loss_path = f"new_loss/{model_type}_model_{model_tag}_5_class_{number_tag}.pkl"
